    ...
    if( debugON == false  ){
        ...
        for( int i=0; i<8; i++ ){
            ...
            switch( i  ){
                case 0: // Temperature
                dec = 1;
                Serial.print( -53 + 175.72 / 65536.0 * ( unsigned int )( value[ i ] / dec ) );
                break;
                case 1: // Humidity
                dec = 1;
                Serial.print( 7 + 125.0 / 65536.0 * ( unsigned int )( value[ i ] / dec ) );
                break;
                case 2: // Light
                dec = 10;
                Serial.print( ( float )( value[ i ] / dec ) );
                break;
                case 3: // Battery
                dec = 10;
                Serial.print( ( float )( value[ i ] / dec ) );
                ...
                case 4: // Solar Panel
                dec = 1;
                Serial.print( ( unsigned int )( value[ i ] / dec ) / 10000 );
                break;
                ...
                case 7: // Noise
                dec = 1;
                // Serial.println( ( unsigned int )( value[ i ] / dec ) );
                for( int id = 0; id < 41; id++  ){
                    if( noise_mVLevels[ id ] > ( unsigned int )( value[ i ] / dec )  ){
                        // Serial.println( id );
                        // Serial.println( noise_mVLevels[ id ] );
                        Serial.print( noise_dBLevels[ id ] );
                        break;
                    }
                };
                break;
                default:
                dec = 1;
                Serial.print( ( unsigned int )( value[ i ] / dec ) );
                break;
            }

            Serial.print( UNITS[ i ] );
        }
        Serial.println( SERVER[ 8 ] );
    }
}
